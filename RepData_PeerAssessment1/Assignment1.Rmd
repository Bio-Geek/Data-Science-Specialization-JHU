...
```
By looking at data we can see that 5-minute interval recorded in format that 
mixes minutes and hours. For example, the the 55-munute mark is recorded as 55, 
but the next value recorded as 100, which means "1 hour and zero minutes", 
so there is a jump in numerical value every hour.
To make it more convenient to work with the time data, we will add a column with
a continuous 5-minutes interval scale.
```{r echo=TRUE}
...
max_steps
```
The interval at 515-520 minutes contains maximal average number of steps, 
that corresponds to 8:35-8:40 AM time-interval.
...
To fill missing values in the dataset we will use average of the 5-min intervals 
across days.
...
We can see that both mean and median values have increased and their values
become equal.
...
From the figure above we can see that there is a difference in activity patterns 
between weekdays and weekends. In particular, walking activity on the weekends 
is more evenly distributed among different intervals then on weekdays. 
